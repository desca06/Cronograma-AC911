...
function esTardanza(horaEntrada: string | null) {
  const hora = normalizarHora(horaEntrada);
  return Boolean(hora) && hora > HORA_LIMITE_ENTRADA;
}
...
function obtenerEstadoCalculado(
  ...
  return esTardanza(horaEntrada) ? "TARDE" : "PRESENTE";
}
...
function formatearMinutos(minutos: number | null | undefined) {
  const total = Math.max(minutos ?? 0, 0);
  if (total === 0) return "—";
...
export default async function AsistenciasPage({
  ...
  const fechaHoy = obtenerFechaGuatemala();
  ...
  const listaAsistencias = await db
    .select({
      ...
      minutosHoraExtra: asistencias.minutosHoraExtra,
  ...
  const asistenciasHoy = listaAsistencias.filter(
    (asistencia) => asistencia.fecha === fechaHoy,
  );
  ...
  const empleadosPresentesHoy = new Set(
    asistenciasHoy
      .filter(
        (asistencia) =>
          Boolean(asistencia.horaEntrada) &&
          asistencia.estado !== "AUSENTE" &&
          asistencia.estado !== "PERMISO" &&
          asistencia.estado !== "VACACIONES",
      )
      .map((asistencia) => asistencia.empleadoId),
  );

  const empleadosTardeHoy = new Set(
    asistenciasHoy
      .filter((asistencia) => esTardanza(asistencia.horaEntrada))
      .map((asistencia) => asistencia.empleadoId),
  );

  const empleadosPermisoHoy = new Set(
    asistenciasHoy
      .filter((asistencia) => asistencia.estado === "PERMISO")
      .map((asistencia) => asistencia.empleadoId),
  );

  const empleadosAusentesHoy = new Set(
    asistenciasHoy
      .filter((asistencia) => asistencia.estado === "AUSENTE")
      .map((asistencia) => asistencia.empleadoId),
  );

  const presentesHoy = empleadosPresentesHoy.size;
  const tardanzasHoy = empleadosTardeHoy.size;
  const permisosHoy = empleadosPermisoHoy.size;
  const ausentesHoy = empleadosAusentesHoy.size;

  const totalMinutosExtraHoy = asistenciasHoy.reduce(
    (total, asistencia) =>
      total + Math.max(asistencia.minutosHoraExtra ?? 0, 0),
    0,
  );
  ...
  const resumen = [
    ...
    {
      titulo: "Horas extra hoy",
      cantidad: formatearMinutos(totalMinutosExtraHoy),
      icono: TimerReset,
      estilos: {
        borde: "border-violet-200",
        fondo: "bg-violet-50",
        fondoIcono: "bg-violet-100",
        textoIcono: "text-violet-700",
        textoCantidad: "text-violet-700",
      },
    },
  ];
      ...
      <section className="space-y-6 p-5 md:p-8">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600">
              Estado actual de las asistencias registradas el {fechaHoy}.
            </p>
          </div>
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-200 p-5">
            ...
            <p className="mt-1 text-sm text-slate-600">
              Listado de entradas, salidas, estados y horas extra del personal.
            </p>
          </div>
            ...
            <table className="min-w-[1180px] w-full">
              <thead className="bg-slate-50">
                <tr className="text-left text-xs font-bold uppercase tracking-wide text-slate-500">
                  ...
                  <th className="px-5 py-4">Hora extra</th>
                  <th className="px-5 py-4">Límite mensual</th>
                ...
                {listaAsistencias.map((asistencia) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="whitespace-nowrap px-5 py-4 font-semibold text-violet-700">
                        {formatearMinutos(asistencia.minutosHoraExtra)}
                      </td>
                      <td className="whitespace-nowrap px-5 py-4">
                        {formatearMinutos(
                          asistencia.limiteMinutosExtraMensuales,
                        )}
                      </td>
                      <td className="whitespace-nowrap px-5 py-4">
                        <span

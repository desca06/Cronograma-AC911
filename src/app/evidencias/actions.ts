...
function revalidarPaginas(
  ...
  revalidatePath(`/evidencias/${trabajoId}`);
...
export async function subirEvidencia(
  ...
  const sesion = await verificarAccesoAlTrabajo(trabajoId);
  const archivo = formData.get("foto");
  const descripcion = obtenerTexto(formData, "descripcion");

  if (!(archivo instanceof File) || archivo.size === 0) {
    redirect(`/evidencias/${trabajoId}?error=archivo`);
  }

  const extension = formatosPermitidos[archivo.type];

  if (!extension) {
    redirect(`/evidencias/${trabajoId}?error=formato`);
  }

  if (archivo.size > TAMANO_MAXIMO) {
    redirect(`/evidencias/${trabajoId}?error=tamano`);
  }
  ...
  const rutaBlob = `evidencias/trabajo-${trabajoId}/${nombreArchivo}`;

  let urlArchivo: string;

  try {
    const blob = await put(rutaBlob, archivo, {
      access: "public",
    ...
    redirect(`/evidencias/${trabajoId}?error=almacenamiento`);
  }
  ...
  try {
    await db.transaction(async (tx) => {
      await tx.insert(evidencias).values({
        ...
        nombreOriginal: archivo.name || nombreArchivo,
      ...
      const supervisores = await tx
        ...
        .from(usuarios)
        .where(eq(usuarios.rol, "SUPERVISOR"));

      if (supervisores.length === 0) {
        return;
      }

      const detalleDescripcion = descripcion
    ...
    console.error("Error guardando evidencia en PostgreSQL:", error);
    redirect(`/evidencias/${trabajoId}?error=base-datos`);
  }

  revalidarPaginas(trabajoId);
  redirect(`/evidencias/${trabajoId}?exito=subida`);
}